...
const GroceriesSlide = () => {
    ...
    useEffect(() => {
        const fetchLaptops = () => {
            const filteredGroceries = data.filter((gro) => gro?.category === "groceries")
            setGroceries(filteredGroceries)
        }
        fetchLaptops()
    }, [])
    const scrollLeft = () => {
        let slider = document.getElementById('slider' + 5);
    ...
    return (
        <div className="flex flex-col justify-center py-2  mx-auto w-full">
            <div>
                <Typography className="text-center text-xl md:text-2xl font-Poppins md:text-start py-2 font-bold md:ml-16">Best Of groceries</Typography>
            </div>
            <div className="flex flex-row justify-center relative">
                ...
                <div id={"slider" + 5} className="flex flex-row overflow-x-scroll relative gap-4 py-2 px-4">
