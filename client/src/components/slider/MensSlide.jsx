...
const MensSlide = () => {
    ...
    useEffect(() => {
        const fetchMens = () => {
            const filteredMobiles = data.filter((men) => men?.category === "mens")
            setMens(filteredMobiles)
        }
        fetchMens()
    }, [])
    const scrollLeft = () => {
    ...
    return (
        <div className="flex flex-col justify-center py-2  mx-auto w-full">
            <div>
                <Typography className="text-center text-xl md:text-2xl font-Poppins md:text-start py-2 font-bold md:ml-16">Best Of Mens</Typography>
            </div>
            <div className="flex flex-row justify-center  relative">
                ...
                <div id={"slider" + 2} className="flex flex-row overflow-x-scroll relative gap-4 py-2 px-4">
                    {mens?.map((item, id) =>
                        <div key={id} className="flex flex-row">
                            <Card className="relative w-48 md:w-64 mx-auto hover:scale-105 cursor-pointer transition-all duration-300 delay-200 ease-in-out" >
                                <Typography className="z-[40] absolute top-1 left-3  bg-red-700 px-2 font-Poppins text-white text-xs">{item?.subcategory}</Typography>
                                <div onClick={() => addWishlist({
                                    ...
                                    rating: item?.rating,
                                    msg: ""
                                })} className='z-[40] absolute right-4 top-0 flex flex-col items-center'>
