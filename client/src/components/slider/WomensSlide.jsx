...
const WomensSlide = () => {
    ...
    useEffect(() => {
        const fetchMens = () => {
            const filteredWomens = data.filter((women) => women?.category === "womens")
            setWomens(filteredWomens)
        }
        fetchMens()
    }, [])
    const scrollLeft = () => {
    ...
    return (
        <div className="flex flex-col justify-center py-2 mx-auto w-full">
            <div>
                <Typography className="text-center text-xl md:text-2xl font-Poppins md:text-start py-2 font-bold md:ml-16">Best Of Womens</Typography>
            </div>
