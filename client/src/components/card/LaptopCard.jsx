...
export default function LaptopCard() {
    ...
    const [filteredLaptops, setfilteredLaptops] = useState(data)
    ...
    const handleFilter = (subcategory) => {
        if (subcategory === "all") {
            setfilteredLaptops(data.filter((item) => item?.category === "laptops"))
        }
        else {
            const laptops = data.filter((item) => item?.subcategory === subcategory)
            setfilteredLaptops(laptops)
        }
    }

    return (
        <div className="bg-white px-2 font-Poppins">
            <div>
                ...
                <Transition.Root show={mobileFiltersOpen} as={Fragment}>
                    <Dialog as="div" className="relative z-40 lg:hidden" onClose={setMobileFiltersOpen}>
                        ...
                        <div className="fixed inset-0 z-[44] flex">
                            <Transition.Child
                                as={Fragment}
                            ...
                            >
                                <Dialog.Panel className="relative ml-auto flex h-full w-full max-w-xs flex-col overflow-y-auto bg-white py-4 pb-12 shadow-xl">
                                    ...
                                    {/* Filters */}
                                    <div className='md:hidden flex flex-col gap-2 justify-center items-center cursor-pointer'>
                                        ...
                                        <div onClick={() => handleFilter("CHUWI")}>
                                            <h4 className='font-medium text-xl'>
                                                Chuwi
                                            </h4>
                                        </div>
                                        <div onClick={() => handleFilter("ACER")}>
                                            <h4 className='font-medium text-xl'>
                                                Acer
                                            </h4>
                                        </div>
                                        <div onClick={() => handleFilter("ASUS")}>
                                            <h4 className='font-medium text-xl'>
                                                Asus
                                            </h4>
                                        </div>
                                        <div onClick={() => handleFilter("MSI")}>
                                            <h4 className='font-medium text-xl'>
                                                Msi
                                            </h4>
                                        </div>
                                        <div onClick={() => handleFilter("APPLE")}>
                                            <h4 className='font-medium text-xl'>
                                                Apple
                                            </h4>
                                        </div>
                                        <div onClick={() => handleFilter("Infinix")}>
                                            <h4 className='font-medium text-xl'>
                                                Infinix
                                            </h4>
                                        </div>
                ...
                <main className="mx-auto max-w-7xl px-4 sm:px-6 lg:px-8">
                    <div className="flex items-baseline justify-between border-b border-gray-200 pb-6 pt-24">
                        <h1 className="text-4xl font-bold tracking-tight text-gray-900">Laptops</h1>
                    ...
                    <section aria-labelledby="products-heading" className="pb-24 pt-6">
                        <div className="grid grid-cols-1 gap-x-8 gap-y-10 lg:grid-cols-4">
                            {/* Filters */}
                            <div className='hidden md:flex flex-col gap-2 justify-start cursor-pointer'>
                                <h3 className='font-bold text-2xl'>Filter By Brands</h3>
                                <hr className='bg-gray-700' />
                                <div onClick={() => handleFilter("all")}>
                                    <h4 className='font-medium text-xl'>
                                    ...
                                    filteredLaptops.map((item, id) =>
                                        <div key={id}>
                                            <Card className="relative w-80 md:w-64 mx-auto hover:scale-105 cursor-pointer transition-all duration-300 delay-200 ease-in-out" >
                                                <div className='z-[40] absolute right-4 top-1 flex flex-col items-center'>
